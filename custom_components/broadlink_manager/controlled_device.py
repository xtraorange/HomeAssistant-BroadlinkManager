...
class ControlledDevice:
    ...
    async def register(self, config_entry):
        """Register the controlled device in Home Assistant."""
    ...
    async def unregister(self):
        """Unregister the controlled device from Home Assistant."""
        device_registry = dr.async_get(self.hass)
        unique_id = f"{self.mac_address}_{self.device_name}"
    ...
    def exists(self):
        """Check if the controlled device exists in the registry."""
        device_registry = dr.async_get(self.hass)
        unique_id = f"{self.mac_address}_{self.device_name}"
        return (
